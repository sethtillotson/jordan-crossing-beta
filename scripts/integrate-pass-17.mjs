#!/usr/bin/env node
/**
 * scripts/integrate-pass-17.mjs
 *
 * Phase 9 — integrates Pass 17 Cross-References, the final "Encounter
 * Edition" wave declaring the corpus's own Cross-Reference project
 * complete (458/458 real Markdown files threaded, 100%), into the
 * REVIEWED layer, growing JC_RECORDS' `reviewed: true` count.
 *
 ...
 *
 * Run from the repository root:  node scripts/integrate-pass-17.mjs
 */
...
const PASS_FILES = [
  { file: 'Pass-17-Cross-References.md', passNum: 17 },
];
...
function main() {
  ...
  console.log(`Parsed ${allEntries.length} entries in Pass 17.`);
  ...
  // 4. Build same-cluster edges from Pass 17 groups.
  ...
  console.log(`Built ${newEdges.length} new same-cluster thread edges from Pass 17.`);
