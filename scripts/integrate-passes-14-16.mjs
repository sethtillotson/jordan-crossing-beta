#!/usr/bin/env node
/**
 * scripts/integrate-passes-14-16.mjs
 *
 * Phase 8 — integrates Pass 14, Pass 15, and Pass 16 Cross-References (the
 * "Encounter Edition" waves continue: doctrinal-spine Seed→Growth→Tablet
 * joints, verbatim-quoted thread joints, lexicon joints with earliest
 * sibling, an expanded chiastic mirror, and a tablet anchor with
 * verifiable phrase — archive coverage 328→419) into the REVIEWED layer,
 * growing JC_RECORDS' `reviewed: true` count.
 *
 ...
 *
 * Run from the repository root:  node scripts/integrate-passes-14-16.mjs
 */
...
const PASS_FILES = [
  { file: 'Pass-14-Cross-References.md', passNum: 14 },
  { file: 'Pass-15-Cross-References.md', passNum: 15 },
  { file: 'Pass-16-Cross-References.md', passNum: 16 },
];
...
function main() {
  ...
  console.log(`Parsed ${allEntries.length} entries across Pass 14-16.`);
  ...
  // 4. Build same-cluster edges from Pass 14-16 groups.
  ...
  console.log(`Built ${newEdges.length} new same-cluster thread edges from Pass 14-16.`);
